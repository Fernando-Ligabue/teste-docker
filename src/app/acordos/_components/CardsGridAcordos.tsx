...
const AccordsComponent: React.FC<AccordsProps> = ({ cards, infoCard }) => {
    return (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 items-center gap-4 auto-rows-max">
                    ...
                    <div className="relative p-6 flex flex-col justify-center">
                        <div className="flex items-start flex-col gap-14">
                            ...
                            <div className="flex-1">
                                <h3 className="text-terciary text-2xl font-bold leading-tight mb-2">
                                    {infoCard.title}
                                </h3>
                            </div>
                        </div>
                ...
                {cards.map((accord, index) => {
                    ...
                    return (
                        <Link
                            key={accord.id}
                            href={`acordo/${accord.link}`}
                            className={getCardClasses(accord.size, index)}
                        >
                            <div
                                className="absolute inset-0 bg-cover bg-center transition-transform duration-300 group-hover:scale-105 my-auto"
                                style={{
                                    backgroundImage: `url(${accord.imageUrl})`,
                                }}
                            />

                            <div className="absolute inset-0" />

                            <div className="absolute inset-0 bg-hover-green opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
...
export default AccordsComponent;